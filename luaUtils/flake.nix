...
outputs = {nixpkgs, ...}: {
        default = let
                ...
                isLuaText = attrs: attrs ? lua && attrs ? isLiteral;
                getLuaText = luaText: with luaText; if isLiteral then lua else addQuotes lua;
                ...
                */
                ...
                if builtins.isAttrs input
                        # luaText input
                        then if isLuaText input
                                then if input.isLiteral
                                        then input.lua
                                        else ''"${input.lua}"''
                ...
                */
                ...
                let
                pathText = if isLuaText path then getLuaText path else builtins.concatStringsSep "." path;
                in
        ...
        in {
                ...
                can result from a simple check of if the options is an attribute set

                maybeLua is shorthand for `either str luaText`
                */
                luaText = (types.submodule ({name, ...}:{
                        options = {
                                ...
                                isLiteral = mkDisableOption "whether to treat <lua> as literal (unquoted) Lua";
                        };
                }));
                maybeLuaText = with types; either str luaText;
                mkLuaTextAttrs = text: {inherit text; isLiteral = true;};

                # Takes in a maybeLuaText instance,
                # returns the Nix string to be used in text output.
                parseMaybeLuaText = input:
                if builtins.isAttrs input
                        then (if input.isLiteral then input.lua else ''"${input.lua}"'')
                else input;
